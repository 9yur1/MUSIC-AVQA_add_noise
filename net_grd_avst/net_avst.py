...
class AVQA_Fusion_Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc_fusion = nn.Linear(1024+1024, 512)   #Kyuri
    ...
    def forward(self, audio, visual_posi, visual_nega, question):
        ...
        ################ add noise ################ Kyuri
        rand_mul_visual = 0;      ## So that the added noise is (0, rand_mul)
        rand_mul_audio = 0;       ## noise 0%
        audio = audio + torch.rand(audio.shape).to(audio.device) * rand_mul_audio
        visual_posi = visual_posi + torch.rand(visual_posi.shape).to(visual_posi.device) * rand_mul_visual
        visual_nega = visual_nega + torch.rand(visual_nega.shape).to(visual_nega.device) * rand_mul_visual
        ...
        v_feat1 = v_feat.permute(0, 2, 1)                            # [B*T, HxW, C]
        visual_feat_posi = nn.functional.normalize(v_feat1, dim=2)   # [B*T, HxW, C]
...
#        visual_feat_grd = torch.matmul(x2_p, visual_feat_posi) ## written by JungUk
        visual_feat_grd = torch.matmul(x2_p, v_feat1) ## writteh by JungUk ## original : visual_feat_posi
        ...
        # print(audio_feat_att.shape) 
        # print(audio_feat_be.shape)

        # print(visual_feat_grd_posi.shape)
        ...
        feat = torch.cat((audio_feat_att + audio_feat_be.mean(dim=-2).squeeze(),
                  ...
                  audio_feat, v_feat), dim=-1)
        '''
        #visual_feat_before_grounding_posi_s = visual_feat_before_grounding_posi.view(8, 512)
        
        #visual_feat_before_grounding_nega_s = visual_feat_before_grounding_nega.view(8, 512)

        #visual_feat_before_grounding_s = torch.cat((visual_feat_before_grounding_posi, visual_feat_before_grounding_nega), dim=1)

        #feat = torch.cat((audio_feat_att+audio_feat_be.mean(dim=-2).squeeze(), visual_feat_att+visual_feat_grd_be.mean(dim=-2).squeeze(), visual_feat_before_grounding_s.view(10,8,512)), dim= -1)
        #visual_feat_before_grounding_posi.view(8, 512).mean(dim=-1)
       
        visual_feat_before_grounding_posi1 =  visual_feat_before_grounding_posi.view(B, -1, 512)
        visual_feat_before_grounding_posi2 = visual_feat_before_grounding_posi1.mean(dim=-2)
        audio_feat_pure1 = audio_feat_pure.view(B, -1, 512).mean(dim=-2)
       

        #print("1:",(audio_feat_att+audio_feat_be.mean(dim=-2).squeeze()).shape)
        #print("2:",(visual_feat_att+visual_feat_grd_be.mean(dim=-2).squeeze()).shape)
        #print("3:",visual_feat_before_grounding_posi2.shape)
        #print("4:",audio_feat_pure1.shape)

        feat = torch.cat((audio_feat_att+audio_feat_be.mean(dim=-2).squeeze(), visual_feat_att+visual_feat_grd_be.mean(dim=-2).squeeze(), visual_feat_before_grounding_posi2, audio_feat_pure1), dim=-1)

        # feat = torch.cat((audio_feat_att+audio_feat_be.mean(dim=-2).squeeze(), visual_feat_att+visual_feat_grd_be.mean(dim=-2).squeeze()), dim=-1)
        # feat = feat.view(8,1024+512)
        



        # origin_feat = torch.cat(visual_feat_before_grounding_posi.view(8,512),audio_feat.view(8,512))
        ...
        combined_feature = torch.mul(feat, qst_feature)  # [B, 512] x [B, 512] #kyuri  --> combined_feature = feat
        ...
        print("audio.max")
        print(audio.max())
        print("visual_posi.max")
        print(visual_posi.max())
        print("visual_nega.max")
        print(visual_nega.max())
'''# Kyuri
